...
$uiPath     = Join-Path $PSScriptRoot "modules\UI.psm1"
$compatPath = Join-Path $PSScriptRoot "modules\Compat.psm1"
if (Test-Path $compatPath) { Import-Module $compatPath -Force -WarningAction SilentlyContinue }
if (Test-Path $uiPath)     { Import-Module $uiPath     -Force -WarningAction SilentlyContinue }
...
function Show-TTS {  Clear-Host
  Write-Host 'Iniciando Audio Biblia (TTS)...' -ForegroundColor Cyan

  $audioApp = Join-Path $PSScriptRoot '..\Biblia-App.ps1'
  if (-not (Test-Path $audioApp)) {
    Write-Host ('[ERROR] No se encontro Biblia-App.ps1 en: {0}' -f $audioApp) -ForegroundColor Red
