...
try { Import-Module (Join-Path $Base "modules\Audio.psm1") -Force -WarningAction SilentlyContinue } catch {}
try { Import-Module (Join-Path $Base "modules\AudioBridge.psm1") -Force -WarningAction SilentlyContinue } catch {}
try { Import-Module (Join-Path $Base "modules\BookMap.psm1") -Force -WarningAction SilentlyContinue } catch {}
...
while ($true) {
  ...
  if ($audioOn -and $toSpeak) {
    SpeakIfAvailable ($bookName + " " + $chapter + ":" + $verse + ". " + $toSpeak)
  }
}
