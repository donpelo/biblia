...
if __name__ == "__main__":
    ...
    try:
        ...
        lp = os.path.join(logs, "desktop_app.log")

        with open(lp, "a", encoding="utf-8") as f:
            f.write(f"[{datetime.datetime.now().isoformat(timespec='seconds')}] START cwd={os.getcwd()} argv={sys.argv}\n")
        ...
        try:
            base = os.path.dirname(__file__)
            lp = os.path.join(base, "logs", "desktop_app.log")
            with open(lp, "a", encoding="utf-8") as f:
                f.write("[EXC] " + repr(e) + "\n")
        ...
        try:
            ...
            lp = os.path.join(base, "logs", "desktop_app.log")
            with open(lp, "a", encoding="utf-8") as f:
                f.write(f"[{datetime.datetime.now().isoformat(timespec='seconds')}] END exitcode={exitcode}\n")
        except Exception:
            pass
